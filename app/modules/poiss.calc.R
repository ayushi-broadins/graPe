...
poisson.z.score <- function(dat,negative.control){
  ...
  #unique_trt['z.prime.score'] <- (unique_trt$trt.lowerqi.per.plate - unique_trt$nc.upperqi.per.plate) / abs(unique_trt$trt.lambdahat.per.plate - unique_trt$nc.lambdahat.per.plate)
  unique_trt['z.prime.score'] <- (unique_trt$trt.lowerqi.per.plate.spline - unique_trt$nc.upperqi.per.plate.spline) / abs(unique_trt$trt.lambdahat.per.plate - unique_trt$nc.lambdahat.per.plate)
  ...
  return(as.data.frame(unique_trt))
}